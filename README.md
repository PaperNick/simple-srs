...
Datasets are generated by build scripts in `backend/scripts/`, grouped by language:

- **Korean** (`scripts/korean/`) - Hangul alphabet and the common Korean words list.
- **Japanese** (`scripts/japanese/`) - Hiragana and Katakana.

Each script writes a dataset JSON file, registers it in `backend/data/datasets.json`, and downloads any audio. Datasets are seeded automatically on start. See the [backend README](backend/README.md) for run instructions and sources.
