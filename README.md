...
Run them from the `backend/` directory (requires the Anki deck path for the words dataset):

```bash
cd backend

# Hangul alphabet (self-contained, downloads audio)
npx tsx scripts/build-hangul.ts

# Korean words (needs the Anki deck .apkg for audio)
npx tsx scripts/build-korean-words-6000.ts path/to/deck.apkg
```

See the [backend README](backend/README.md) for the Korean words dataset's sources (the TOPIK frequency list + Anki deck audio).
