...
Express + SQLite API that powers Simple SRS. It exposes the REST endpoints and runs the spaced-repetition (SRS) scheduling for vocabulary, plus the no-SRS character practice grader.
...
Datasets are declared in `data/datasets.json`. Each entry is one JSON **file** that becomes its own deck in the UI.
...
All deck files share **one unified item schema**:
```
...
On first start, the backend creates the local database (`data/simple_srs.sqlite`) and imports each dataset deck file (one row per entry), tagging it with its `deck` id.
Add a new deck by adding a JSON file + an entry in `datasets.json` and it will appear in the app.
...
| POST   | `/api/review/answer`  | Grade a review answer (feedback only)              |
| POST   | `/api/review/schedule`| Advance/reset an item's SRS stage after a review   |
