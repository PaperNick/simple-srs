...
Datasets are declared in [`data/datasets.json`](data/datasets.json). Each entry is one JSON **file** that becomes its own card in the UI.
...
- `badge` / `description` - the card's label and subtitle. The total count is always shown with the unit `items`.
...
The Hangul alphabet dataset is generated by `scripts/build-hangul.js` (which also downloads its audio).
...
Add a new dataset by adding a JSON file + an entry in `datasets.json` - it appears as its own card.
...
| GET    | `/api/datasets`        | Dataset cards (name, mode, counts)                 |
...
The Hangul alphabet dataset is regenerable in the same way with `node scripts/build-hangul.js`.
